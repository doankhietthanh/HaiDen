<%@ Page Title="Collection Detail " Language="C#" MasterPageFile="~/Admin/MasterPage.Master" AutoEventWireup="true" CodeBehind="Wedding_Dress.aspx.cs" Inherits="HaiDen.Admin.Wedding_Dress" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
        ...
        <table id="data-table-command" class="table table-striped table-vmiddle">
            <thead>
                <tr>
                    ...
                    <th data-column-id="name" data-order="desc">Tên váy</th>
                    <th data-column-id="des" data-order="desc">Mô tả</th>
                    <th data-column-id="price" data-order="desc">Mã số </th>
                    <th data-column-id="collect" data-order="desc">Thuộc Bộ sưu tập </th>
                    <th data-column-id="commands" data-formatter="commands" data-sortable="false">Action</th>
                </tr>
            </thead>
            <tbody class="list_dress">
                <tr>
                    <td></td>
                </tr>
            </tbody>
                    ...
                    <div class="col-sm-12">
                        <form id="frm">
                            <div class="form-group">
                                <label>Tên bộ váy cưới </label>
                                <div class="fg-line">
                                    <input type="text" class="form-control iname" required maxlength="100" minlength="2" placeholder="Nhâp Tên Bộ Váy... ">
                                </div>
                            </div>
                            <div class="form-group">
                                <label>Mã Số </label>
                                <div class="fg-line">
                                    <input type="text" class="form-control iprice" required placeholder="Nhâp mã số... ">
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="fg-line">
                                    <label>Thuộc collection nào</label>
                                    <div class="select-not fg-line-c">
                                        <select class="form-control t-white icate">
                                            <option value="-1">Chọn collection ..</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="fg-line">
                                    <label>Mô tả</label>
                                    <textarea class="form-control auto-size ides" required maxlength="300" placeholder="Nhập mô tả ..."> </textarea>
                                </div>
                            </div>
                        </form>
                    </div>
    ...
    <%-- POPUP QUẢN LÝ HÌNH ẢNH  --%>
    <div class="modal fade" data-modal-color="lightgreen" id="add_img" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title">UPLOAD HÌNH DRESS</h3>
                    <button type="button" class="btn btn-link" data-dismiss="modal" style="position: absolute; right: 10px; top: 15px; color: #000;">X</button>
                </div>
                <div class="modal-body_1">
                    <iframe id="fr_img" src="Images_Product.aspx?id=-1" style="border: none;" width="100%" height="500"></iframe>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder2" runat="Server">
    ...
    <script type="text/javascript">
        ...
        //===================LOAD LIST DRESS CATE =========================

        function load_dress() {

            var Shoplist = [{}];
            var shoplistSt = escape(JSON.stringify(Shoplist));
            param = { "stpe": 25, "shoplistSt": shoplistSt };
            postAjax(param, load_dress_result, "html");
        }

        function load_dress_result(e) {
            if (e != "err") {
                if (reload_key == 1) {
                ...
                var grid = $("#data-table-command").bootgrid({
                    ...
                    formatters: {
                        "commands": function (column, row) {
                            return "<button type=\"button\" class=\"btn btn-icon cmd-edit waves-effect waves-circle btn-warning\" data-row-id=\"" + row.id + "\"><span class=\"zmdi zmdi-edit\"></span></button>" +
                                "<button type=\"button\" class=\"btn btn-icon cmd-images waves-effect waves-circle btn-success \" data-row-id=\"" + row.id + "\"><span class=\"zmdi zmdi-image\"></span></button>" +
                    ...
                    grid.find(".cmd-edit").on("click", function (e) {
                        ...
                        $('.modal-title').html('Sửa Mẫu Váy Số: ' + id);
                    ...
                    grid.find(".cmd-images").on("click", function (e) {
                        var id = $(this).data("row-id");
                        console.log(e)
                        console.log(id);
                        $('#add_img').modal({ backdrop: 'static', keyboard: false });
                        img_edit(id);
                    });
                });
        ...
        function img_edit(id) {
            var hrf = "../Admin/Images_Product.aspx?id=" + id;
            $("#fr_img").attr('src', hrf);
        }
        ...
        function load_list_cate() {
            ...
            var Shoplist = [{}];
            var shoplistSt = escape(JSON.stringify(Shoplist));
            var param = { "stpe": 26, "shoplistSt": shoplistSt };
            postAjax(param, load_list_cate_result, "html");
        }
        function load_list_cate_result(e) {
            if (e != 'err') {
                $('.icate').html('<option value="0" >Chọn collection..</option>' + e);

                //}
            }
        }

        //===================INSERT UPDATE DRESS CATE  =====================
        function insert_update_dress() {
            ...
            var shoplistSt = escape(JSON.stringify(Shoplist));
            var param = { "stpe": 27, "shoplistSt": shoplistSt };
            postAjax(param, insert_update_dress_result, "html");
        }
        function insert_update_dress_result(e) {
            if (e != 'err') {
                reload_key = 1;
                reload();
        ...
        function delete_dress_id(id) {
            ...
            swal({
                ...
                if (isConfirm) {
                    var Shoplist = [{ "id": id }];
                    var shoplistSt = escape(JSON.stringify(Shoplist));
                    var param = { "stpe": 28, "shoplistSt": shoplistSt };
        ...
        function load_dress_id(id) {

            var Shoplist = [{ "id": id }];
            var shoplistSt = escape(JSON.stringify(Shoplist));
            var param = { "stpe": 9, "shoplistSt": shoplistSt };
            postAjax_list(param, load_dress_id_result, "html");
        }
        function load_dress_id_result(e) {
