<%@ Page Title="Head Image" Language="C#" MasterPageFile="~/Admin/MasterPage.Master" AutoEventWireup="true" CodeBehind="Banner.aspx.cs" Inherits="HaiDen.Admin.Banner" %>
        ...
        <table id="data-table-command" class="table table-striped table-vmiddle">
            <thead>
                <tr>
                    ...
                    <th data-column-id="tieude" data-order="desc">Tiêu đề</th>
                    <th data-column-id="mota" data-order="desc">Mô tả</th>
                    <th data-column-id="commands" data-formatter="commands" data-sortable="false">Action</th>
                </tr>
            </thead>
            <tbody class="list_banner">
                <tr>
                    <td></td>
                </tr>
            </tbody>
                    ...
                    <div class="col-sm-6">
                        <form id="frm">
                            <div class="form-group">
                                <label>Tiêu đề</label>
                                <div class="fg-line">
                                    <input type="text" class="form-control ititle" required maxlength="200" minlength="2" value="" placeholder="Tiêu đề">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="fg-line">
                                    <label>Mô tả</label>
                                    <input type="text" class="form-control ides" required value="" required maxlength="300" minlength="0" placeholder="Mô tả">
                                </div>
                            </div>
                            ...
                            <div class="form-group ">
                                ...
                                <div class="fg-line">
                                    <input type="text" class="form-control ilink" required maxlength="200" minlength="2" value="" placeholder="Đường dẫn hình">
                                </div>
                            </div>

                                <label class="note">Kích thước ảnh Head image: 1020px X 765px (cứ nhân theo tỷ lệ này lên là được) </label>
                        </form>
                    </div>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder2" runat="Server">
    ...
    <script type="text/javascript">
        var reload_key = 0;
        var path = '../Assets/HinhAnh/Banners/';
        $(document).ready(function () {
            load_banner();
            $('.btn-save').on('click', function () {
                var isValid = $("#frm").valid();
                if (isValid) {
                    insert_update_banner();
                }
            });
            uploadimg('APP_LIST');
        ...
        function load_banner() {
            ...
            param = { "stpe": 4, "shoplistSt": shoplistSt };
            postAjax(param, load_banner_result, "html");
        }
        function load_banner_result(e) {
            if (e != "err") {
                if (reload_key == 1) {
                ...
                var grid = $("#data-table-command").bootgrid({
                    ...
                    templates: {
                        header: "<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div class=\"col-sm-12 actionBar\"><button class=\"btn btn-success waves-effect btn-custom\">Thêm Banner</button><p class=\"{{css.search}}\"></p><p class=\"{{css.actions}}\"></p></div></div></div>"
                    },
                    formatters: {
                    ...
                    grid.find(".cmd-edit").on("click", function (e) {
                        ...
                        $('.modal-title').html('Sửa thông tin banner số: ' + id);
                ...
                $('.btn-custom ').on('click', function () {
                    $('#preventClick').modal({ backdrop: 'static', keyboard: false });
                    $('.modal-title').html('Thêm Banner Mới');
        ...
        function insert_update_banner() {
            ...
            var shoplistSt = escape(JSON.stringify(Shoplist));
            var param = { "stpe": 5, "shoplistSt": shoplistSt };
            postAjax(param, insert_update_banner_result, "html");
        }
        function insert_update_banner_result(e) {
            if (e != 'err') {
                reload_key = 1;
                reload();
        ...
        function load_banner_id(id) {

            var Shoplist = [{ "id": id }];
            var shoplistSt = escape(JSON.stringify(Shoplist));
            var param = { "stpe": 2, "shoplistSt": shoplistSt };
            postAjax_list(param, load_banner_id_result, "html");
        }
        function load_banner_id_result(e) {
            for (var i in e) {
                var dr = e[0];

                var title = $('.ititle').val(dr.title),
                    des = $('.ides').val(dr.des),
                    key = $('.ikey').val(dr.key),
                    link = $('.ilink').val(dr.link);
